      ...
      {cartItems.length === 0 ? (
        <div className='empty-cart'>
          <div className='cart-img-container'>
            <div className='cart-img'><ShoppingCartIcon /></div>
          </div>
          <h2>Tu carrito está vacío</h2>
          <p>¿Aún no te has decidido?. Tenemos servicios y productos que te encantaran, vuelve a la pagina de la tienda para comprobarlo</p>
         <Link to={'/'}> <button className='btn-tienda'>Volver a la tienda</button></Link>
        </div>
      ) : (
        <section className='cart'>
            ...
            <div key={item.id} className='cart-item'>
              <div className='item-container'>
                <div>
                  ...
                  <p className='item-price'>$USD {item.price * item.quantity}</p>
                </div>
                <div>
              ...
              <div className='item-actions'>
                <div className='action-circle trash' onClick={() => handleDeleteItem(item.id)}><DeleteIcon /></div>
                <div className='quantity'>{item.quantity}</div>
                <div className='action-circle' onClick={() => handleQuantityChange(item.id, '+')}>
                  <AddIcon />
                </div>
                <div className='action-circle' onClick={() => handleQuantityChange(item.id, '-')}>
                  <RemoveIcon />
          ...
          <div className='total-compra'>
           <p> <span>El Total de tu compra es:</span> $USD {total}</p> 
